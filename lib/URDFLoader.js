...
class URDFLoader {
    ...
    parse(content) {
        ...
        function processLink(link, visualMap, colliderMap, target = null) {
            ...
            // !!!!!
            // console.log("processLink", link, parseVisual, parseCollision)
            ...
            const intertialNodes = children.filter(n => n.nodeName.toLowerCase() === 'inertial');
            if (intertialNodes.length > 0) {

                for (let i in intertialNodes[0].children) {
                    if (intertialNodes[0].children[i].nodeName == "mass") {
                        mass =  intertialNodes[0].children[i].getAttribute('value');
                        // console.log("i", i, intertialNodes[0].children[i].nodeName);
                    }
                }
                // mass = intertialNodes[0].getAttribute('mass');
            }

            // console.log("target.name", target.name, mass);
            ...
            if (parseCollision) {
                ...
                collisionNodes.forEach(cn => {
                    ...
                    //     console.log("parseCollision", cn.getAttribute('name'))
                    // }
        ...
        function processLinkElement(vn, materialMap = {}) {
            ...
            children.forEach(n => {
                ...
                if (type === 'geometry') {
                    ...
                    // !!!!!
                    // console.log("processLinkElement", type, geoType, "isCollisionNode", isCollisionNode)
                    ...
                    if (geoType === 'mesh') {
                        ...
                        primitiveModel.geometry = new THREE.BoxGeometry(size[0]-0.001, size[1]-0.001, size[2]-0.001);
                        // primitiveModel.geometry = new THREE.BoxBufferGeometry(1, 1, 1);

                        primitiveModel.material = material;

                        // !!!!!
                        // console.log("box", size, primitiveModel.scale, primitiveModel)
                        ...
                        primitiveModel.geometry = new THREE.SphereGeometry(radius, 30, 30);
                        primitiveModel.material = material;
                        ...
                        primitiveModel.geometry = new THREE.CylinderGeometry(radius, radius, length, 30);
                        primitiveModel.material = material;
                    ...
                    // console.log("origin", xyz[0], xyz[1], xyz[2], rpy);
                }
